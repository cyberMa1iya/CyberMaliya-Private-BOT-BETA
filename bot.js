...
const {Message, StringSession, Image, Video} = require('./MaliyaBot/');
...
// ════════════════════SQL🍁🍁
const MALIYAHDB = config.DATABASE.define('MALIYAH', {
    info: {
...
async function MALIYAH () {
    await config.DATABASE.sync();
    var StrSes_Db = await MALIYAHDB.findAll({
        where: {
          info: 'StringSession'
        }
    });
...
// ════════════════════WA CONNECTION🍁🍁🍁
    ...
    conn.browserDescription = ["MALIYAH", "Safari", '4.0.0']
    ...
    conn.on ('credentials-updated', async () => {
        ...
        if (StrSes_Db.length < 1) {
            await MALIYAHDB.create({ info: "StringSession", value: Session.createStringSession(authInfo) });
        } else {
...
// ════════════════════PLUGGINS SUCCESS🍁🍁🍁
        console.log(
            chalk.green.bold(' ⎝🛡️ MALIYAH 🛡️⎠ WHATSAPP BOT WORKING!▷')
       );
        
        console.log(
            chalk.blueBright.italic('⎝🛡️ MALIYAH 🛡️⎠ WhatsApp User Bot v4.0.0')
        );
        
         if (config.LANG == 'EN') {
             await conn.sendMessage(conn.user.jid, fs.readFileSync("./src/cd6032c65c27e0510ddad.jpg"), MessageType.image, { caption: `⎝🛡️ Hellow !! ${conn.user.name}! \n\n*🖲️ Welcome To ⎝🛡️ MALIYAH 🛡️⎠ WhatsApp User Bot  :│🖲️*\n\n\n Your Bot Working  As ${config.WORKTYPE} 🖲️.\n\n*🖲️│⎝🛡️ MALIYAH 🛡️⎠ WORKING Your Account*\n\n*🖲️▷ Use the 🚀.MALIYAH command to get bot menu...*\n\n\n*🖲️ ⎝🛡️ MALIYAH 🛡️⎠ is a powerfull WhatsApp robot developed by Achi Fernando (Achiya).*\n\n*🚀 This is your LOG number. Avoid using the command here.\n\n🖲️ .up Command use for new items*\n\n`});
             
         } else if (config.LANG == 'SI') {
             await conn.sendMessage(conn.user.jid, fs.readFileSync("./src/cd6032c65c27e0510ddad.jpg"), MessageType.image, { caption: `⎝🛡️ අයුබොවන් !! ${conn.user.name}! \n\n*🖲️ සාදරයෙන් ⎝🛡️ MALIYAH 🛡️⎠ WhatsApp User Bot  වෙත පිලිගන්නවා :│🖲️*\n\n\n ඔබේ Bot ${config.WORKTYPE} ලෙස ක්‍රියාකරයි.\n\n*🖲️│⎝🛡️ MALIYAH 🛡️⎠ ඔබගේ ගිණුමේ දැන් සක්‍රියයි*\n\n*🖲️ ⎝🛡️ MALIYAH 🛡️⎠ bot සම්පූර්න මෙනුව ලබා ගැනීමට 🖲️.MALIYAH විධානය භාවිතා කරන්න...*\n\n\n*🖲️ ⎝🛡️ MALIYAH 🛡️⎠ යනූ සීඝ්‍රයෙන් වර්ධනය වන Whatsapp රොබෝවෙකි..⎝🛡️ MALIYAH 🛡️⎠ වෙත ලැබෙන නව අංග හා යතාවත්කාලින කිරිම් ලබා ගැනීමට🖲️ .up විධානය භාවිතා කරන්න..*\n\n*🖲️ මෙය ඔබගේ LOG අංකයයි.මෙහි විධාන භාවිතයෙන් වළකින්න.*\n\n`});
             
         } else {
             await conn.sendMessage(conn.user.jid, fs.readFileSync("./src/cd6032c65c27e0510ddad.jpg"), MessageType.image, { caption: `⎝🛡️ Hellow !!  ${conn.user.name}! \\nn*🖲️ Welcome To ⎝🛡️ MALIYAH 🛡️⎠ WhatsApp User Bot  :│🖲️*\n\n\n Your Bot Working  As ${config.WORKTYPE} 🖲️.\n\n*🖲️ │⎝🛡️ MALIYAH 🛡️⎠ WORKING Your Account*\n\n*🖲️▷ Use the 🚀.MALIYAH command to get bot menu...*\n\n\n*🖲️ ⎝🛡️ MALIYAH 🛡️⎠ is a powerfull WhatsApp robot developed by Achi Fernando (Achiya).*\n\n*🚀 This is your LOG number. Avoid using the command here.\n\n🖲️ .up Command use for new items*\n\n`});
        }
     });
    ...
    setInterval(async () => { 
        if (config.AUTOBIO == 'true') {
            if (conn.user.jid.startsWith('90')) { 
                ...
                const biography = '📅 ' + utch + '\n⌚ ' + ov_time + '\n\nPOWERD BY ⎝🛡️ MALIYAH 🛡️⎠ WhatsApp User Bot '
...
*/
                        try {
                            ...
                            if (config.LANG == 'EN') {
                                await conn.sendMessage(conn.user.jid, fs.readFileSync("./src/cd6032c65c27e0510ddad.jpg"), MessageType.image, { caption: '⎝🛡️ MALIYAH 🛡️⎠ WhatsApp User Bot  WORKING AS '+config.WORKTYPE+'!!\n\n▷ _This is your LOG number Dont Try Command here_\n▷Also You Can join Our Support group More Help.\n_🖲️Support 01▷ https://chat.whatsapp.com/EmxfOklzLVIIyDEKPx4IYj\n\n*Error:* ```' + error + '```\n\n' });
                                
                            } else if (config.LANG == 'SI') {
                                await conn.sendMessage(conn.user.jid, fs.readFileSync("./src/cd6032c65c27e0510ddad.jpg"), MessageType.image, { caption: '⎝🛡️ MALIYAH 🛡️⎠ WhatsApp User Bot   '+config.WORKTYPE+' ලෙස ක්‍රියා කරයි!!\n\n▷ _මෙය ඔබගේ LOG අංකයයි මෙහි විධන භාවිතයෙන් වළකින්න_\n▷ඔබට යම් ගැටලුවක් ඇත්නම් අපගේ සහය සමූහට ලිවිය හැක.\nf_🖲️Support 01▷ https://chat.whatsapp.com/EmxfOklzLVIIyDEKPx4IYj\n\n*දෝෂය:* ```' + error + '```\n\n' });
